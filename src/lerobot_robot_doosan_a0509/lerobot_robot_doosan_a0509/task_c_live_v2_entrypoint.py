"""Fail-fast entry point for externally gated Task-C handoff V2."""
...
def require_task_c_live_v2_arguments(arguments: Sequence[str]) -> None:
    required = {
    ...
    for name, expected in required.items():
        ...
        if actual is None or actual.lower() != expected:
            raise ValueError(f"Task-C live V2 requires {name}={expected}")
    for required_path in (
        "--strategy.runtime_manifest",
        "--strategy.handoff_episode_manifest",
        "--strategy.checkpoint_b",
        "--strategy.event_jsonl_path",
        "--strategy.v2_trace_jsonl_path",
    ):
        if not _argument_value(arguments, required_path):
            raise ValueError(f"Task-C live V2 requires {required_path}")
    require_async_fifo_arguments(list(arguments))


def main() -> None:
    try:
        require_task_c_live_v2_arguments(sys.argv[1:])
    except ValueError as exc:
        raise SystemExit(str(exc)) from exc
    main_cpu_set = pin_current_thread_from_env("LEROBOT_A0509_MAIN_CPU_SET")
    if main_cpu_set is not None:
        print(f"Task-C live V2 main/camera threads pinned to CPUs {main_cpu_set}")
    install_act_async_chunk_compat()
    install_task_c_live_strategy()
    install_task_c_live_v2_strategy()
