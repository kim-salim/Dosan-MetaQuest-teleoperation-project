"""Fail-fast entry point for one externally gated Task-C live transition."""
...
def require_task_c_live_arguments(arguments: Sequence[str]) -> None:
    required = {
    ...
    for name, expected in required.items():
        ...
        if actual is None or actual.lower() != expected:
            raise ValueError(f"Task-C live requires {name}={expected}")
    require_async_fifo_arguments(list(arguments))


def main() -> None:
    try:
        require_task_c_live_arguments(sys.argv[1:])
    except ValueError as exc:
        raise SystemExit(str(exc)) from exc
    main_cpu_set = pin_current_thread_from_env("LEROBOT_A0509_MAIN_CPU_SET")
    if main_cpu_set is not None:
        print(f"Task-C live main/camera threads pinned to CPUs {main_cpu_set}")
    install_act_async_chunk_compat()
    install_task_c_live_strategy()
