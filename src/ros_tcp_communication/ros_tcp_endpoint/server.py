    ...
    def handle_syscommand(self, topic, data):
        ...
        if function is None:
            ...
            try:
                ...
                if trailing:
                    self.logwarn(
                        "Ignoring trailing data in SysCommand '{}': {!r}".format(topic, trailing)
                    )
            except json.JSONDecodeError as exc:
                self.logerr(
                    "Failed to decode SysCommand '{}': {}; raw={!r}".format(
                        topic, exc, message_json
                    )
                )
                self.send_unity_error(
                    "Failed to decode SysCommand '{}': {}".format(topic, exc)
                )
                return
            function(**params)
