#!/usr/bin/env python3
"""Promote the exact-O1 T4->T7 semantic-local edge into spatial Level-2 v10.

The successful v9 registry remains read-only. Only T4.open_drawer ->
...
DEFAULT_BASE = ARTIFACT_ROOT / "edge_registry_level2_spatial_v9.json"
DEFAULT_OVERLAYS = (
    ARTIFACT_ROOT
    / "edges/t4_open_drawer__to__t7_acquire_from_drawer_top/"
    "level2_registry_overlay.json",
)
DEFAULT_OUTPUT = ARTIFACT_ROOT / "edge_registry_level2_spatial_v10.json"
DEFAULT_COMPOSITION_ID = (
    "a0509_t1_t8_spatial_floor_semantic_local_t4_t2_t7_level2_v10_20260903"
)
EXPECTED_EDGES = {
...
def main() -> None:
    ...
    if actual != EXPECTED_EDGES or len(applied) != len(EXPECTED_EDGES):
        raise RuntimeError(
            f"T4->T7 exact-exit overlay differs: expected={EXPECTED_EDGES} "
            f"actual={actual}"
        )
    for edge in edges:
    ...
    t4_selections = [
        *contract.get("t4_open_exact_exit_selection", ()),
        *selections,
    ]
    contract.update(
        {
            ...
            "t4_open_exact_exit_selection": t4_selections,
    ...
    print(
        "T4_OPEN_T7_SEMANTIC_LOCAL_LEVEL2_V10_PROMOTED "
