#!/usr/bin/env python3
"""Promote the exact-O1 T4->T2 semantic-local edge into spatial Level-2 v9.

The successful v7 registry remains read-only. Only T4.open_drawer ->
T2.acquire_from_floor is replaced, after validating its command-free geometry
manifest and matching recorded-observation policy shadow.
"""
...
DEFAULT_BASE = ARTIFACT_ROOT / "edge_registry_level2_spatial_v7.json"
DEFAULT_OVERLAYS = (
    ARTIFACT_ROOT
    / "edges/t4_open_drawer__to__t2_acquire_from_floor/"
    "level2_registry_overlay.json",
)
DEFAULT_OUTPUT = ARTIFACT_ROOT / "edge_registry_level2_spatial_v9.json"
DEFAULT_COMPOSITION_ID = (
    "a0509_t1_t8_spatial_floor_semantic_local_t4_t2_level2_v9_20260903"
)
EXPECTED_EDGES = {
...
def main() -> None:
    ...
    if actual != EXPECTED_EDGES or len(applied) != len(EXPECTED_EDGES):
        raise RuntimeError(
            f"T4 exact-exit overlay differs: expected={EXPECTED_EDGES} "
            f"actual={actual}"
        )
    for edge in edges:
    ...
    contract.update(
        {
            ...
            "semantic_local_scope": (
                "held-object transport collars plus exact-O1 T1/T4 open exits"
            ),
            "eligible_edge_count": semantic_count,
            "exact_semantic_exit_edge_count": exact_exit_count,
            "t4_open_exact_exit_selection": selections,
    ...
    print(
        "T4_OPEN_T2_SEMANTIC_LOCAL_LEVEL2_V9_PROMOTED "
